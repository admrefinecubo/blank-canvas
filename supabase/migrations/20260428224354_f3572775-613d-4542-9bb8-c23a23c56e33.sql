...
BEGIN
  ...
  WHERE c.loja_id = v_loja_id
    ...
    AND (
      v_q IS NULL
      OR c.nome_exibicao ILIKE '%' || v_q || '%'
      OR c.descricao ILIKE '%' || v_q || '%'
      OR c.tags ILIKE '%' || v_q || '%'
      OR c.sku ILIKE '%' || v_q || '%'
    )
...
REVOKE ALL ON FUNCTION public.ia_catalogo_buscar(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ia_catalogo_buscar(JSONB) TO service_role;