...
DROP FUNCTION IF EXISTS public.decrementar_estoque(uuid, uuid, integer);
DROP FUNCTION IF EXISTS public.decrementar_estoque(uuid, integer, uuid);
DROP FUNCTION IF EXISTS public.recompor_estoque(uuid, integer, uuid);
...
BEGIN
  ...
  IF p_variacao_id IS NOT NULL THEN
    ...
    IF v_estoque_anterior IS NULL THEN
      RETURN jsonb_build_object('success', false, 'error', 'Variacao nao encontrada');
    END IF;

    v_estoque_novo := GREATEST(v_estoque_anterior - p_quantidade, 0);

    UPDATE produto_variacoes
    SET estoque = v_estoque_novo, updated_at = NOW()
  ...
  ELSE
    ...
    IF v_estoque_anterior IS NULL THEN
      RETURN jsonb_build_object('success', false, 'error', 'Produto nao encontrado para estoque');
    END IF;
  ...
  INSERT INTO estoque_movimentacoes (
    ...
    v_loja_id, p_produto_id, p_variacao_id, 'venda',
    COALESCE(p_referencia_id, p_produto_id::text),
  ...
  RETURN jsonb_build_object(
    ...
    'quantidade_decrementada', p_quantidade,
    'estoque_insuficiente', v_estoque_anterior < p_quantidade
  );
END;
$$;
...
BEGIN
  ...
  INSERT INTO estoque_movimentacoes (
    ...
    v_loja_id, p_produto_id, p_variacao_id, 'ajuste',
    COALESCE(p_referencia_id, p_produto_id::text),
    v_estoque_anterior, v_estoque_novo, p_quantidade,
    NOW()
  );
  ...
  IF p_event_id IS NOT NULL THEN
    ...
    IF v_webhook_event_id IS NULL THEN
      RETURN jsonb_build_object(
        'success', true,
        'duplicado', true,
        'message', 'Evento ja processado (event_id duplicado)'
      );
    END IF;
  END IF;
...
REVOKE ALL ON FUNCTION public.decrementar_estoque(uuid, integer, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.decrementar_estoque(uuid, integer, uuid, text) TO service_role;

REVOKE ALL ON FUNCTION public.recompor_estoque(uuid, integer, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.recompor_estoque(uuid, integer, uuid, text) TO service_role;

REVOKE ALL ON FUNCTION public.fn_ecommerce_order_process FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_ecommerce_order_process TO service_role;