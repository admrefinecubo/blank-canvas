...
export default function AdminLojaFollowups() {
  ...
  const { data: followups, isLoading } = useQuery({
    queryKey: ["admin-loja-followups", lojaId],
    queryFn: async () => {
  ...
  const { data: leads } = useQuery({
    queryKey: ["admin-loja-followups-leads", lojaId],
    queryFn: async () => {
  ...
  const filtered = useMemo(() => {
    return (followups ?? []).filter((item) => {
      ...
      const matchesStatus =
        statusFiltro === "todos" ||
        (statusFiltro === "enviado" ? !!item.enviado : !item.enviado);
  ...
  const markSentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-loja-followups", lojaId] });
  ...
  return (
    <AdminLojaSectionLayout
      ...
      title="Follow-ups"
      description="Gerencie os retornos agendados e acompanhe o que já foi enviado."
      actions={
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Badge variant={item.enviado ? "default" : "secondary"}>{item.enviado ? "Enviado" : "Pendente"}</Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            size="sm"
                            className="gap-2"
                            disabled={!!item.enviado || markSentMutation.isPending}
