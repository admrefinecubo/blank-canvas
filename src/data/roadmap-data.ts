...
export const checklistBlocks: ChecklistBlock[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { number: 20, functionality: "Cross-sell / Upsell de produtos complementares", status: "done", priority: "Média", observations: "Prompt do agente orienta cross-sell por ambiente. Lógica automática via Claude Code." },
      { number: 21, functionality: "Pós-venda", status: "in_progress", priority: "Média", observations: "Tabela post_sale_contacts + automação em finalização" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { number: 30, functionality: "Busca semântica no catálogo via RAG (vetorização por produto)", status: "done", priority: "Alta", observations: "Extension vector + match_produtos testado end-to-end via Claude Code" },
      ...
      { number: 37, functionality: "Re-indexação automática ao atualizar produto no catálogo", status: "done", priority: "Alta", observations: "WF-11 com trigger automático configurado via Claude Code" },
    ],
  },
  {
    ...
    items: [
      { number: 38, functionality: "Shopify", status: "in_progress", priority: "Alta", observations: "Integração em finalização via Claude Code" },
      { number: 39, functionality: "Nuvemshop", status: "in_progress", priority: "Alta", observations: "Integração em finalização via Claude Code" },
      { number: 40, functionality: "Tray", status: "pending", priority: "Média", observations: "Fase 2 — após lançamento" },
      { number: 41, functionality: "VTEX", status: "pending", priority: "Média", observations: "Fase 2 — após lançamento" },
      { number: 42, functionality: "VendiZap", status: "pending", priority: "Média", observations: "Fase 2 — após lançamento" },
      ...
      { number: 45, functionality: "Links de checkout por variação de produto", status: "in_progress", priority: "Alta", observations: "Campo checkout_url por produto — em ajuste final" },
      { number: 46, functionality: "Botão interativo 'Comprar Agora' via WhatsApp Business API", status: "in_progress", priority: "Alta", observations: "Depende da API do WhatsApp Business — em configuração" },
    ],
  },
  {
    ...
    items: [
      ...
      { number: 48, functionality: "enviar_midia_whatsapp — fotos e vídeos direto no chat (com legenda e delay)", status: "done", priority: "Alta", observations: "Edge function corrigida via Claude Code — envio funcional" },
      ...
      { number: 50, functionality: "gerar_orcamento_formal — PDF com itens, descontos, pagamento e validade", status: "done", priority: "Alta", observations: "WF-09 gera orçamento HTML/PDF. Finalizado via Claude Code." },
      ...
      { number: 54, functionality: "gerar_cobranca — gerar link de pagamento", status: "in_progress", priority: "Alta", observations: "WF-10 preparado — aguardando configuração do gateway de pagamento do cliente" },
  ...
  },
  {
    ...
    items: [
      { number: 56, functionality: "Follow-up para clientes que interagiram pouco (msgs iniciais sem avançar)", status: "done", priority: "Alta", observations: "WF-02 cron refinado via Claude Code" },
      { number: 57, functionality: "Follow-up de carrinho abandonado (com oferta de desconto 5% ou 10%)", status: "done", priority: "Alta", observations: "WF-02 + desconto_carrinho_abandonado configurável" },
      { number: 58, functionality: "Follow-up de promoção não respondida (com oferta de desconto 5% ou 10%)", status: "done", priority: "Alta", observations: "WF-02 + desconto_promocao_nao_respondida configurável" },
      { number: 59, functionality: "Follow-up de orçamento pendente", status: "done", priority: "Alta", observations: "WF-02 + desconto_followup_orcamento configurável" },
      { number: 60, functionality: "Follow-up pós-visita à loja", status: "in_progress", priority: "Média", observations: "Em finalização — trigger pós status 'realizada' na visita" },
      { number: 61, functionality: "Follow-up de medidas do ambiente (cliente pediu tempo para medir)", status: "in_progress", priority: "Média", observations: "Em finalização via Claude Code" },
      { number: 62, functionality: "Follow-up pós-venda (avaliação da compra e da loja)", status: "in_progress", priority: "Média", observations: "Em finalização — integrado com NPS" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { number: 72, functionality: "Envio de fotos do produto com legenda no WhatsApp", status: "done", priority: "Alta", observations: "WF-04/WF-06 corrigidos via Claude Code — envio funcional" },
      { number: 73, functionality: "Envio de vídeos demonstrativos do produto no WhatsApp", status: "done", priority: "Alta", observations: "WF-06 suporta vídeo — testado end-to-end" },
      { number: 74, functionality: "Delay entre envios (anti-spam, 1,5s entre itens)", status: "done", priority: "Alta", observations: "WF-04 node Wait validado" },
      { number: 75, functionality: "Ordem correta de processamento: mídia → texto → CRM", status: "done", priority: "Alta", observations: "Pipeline WF-04 corrigido via Claude Code" },
      { number: 76, functionality: "Botão interativo 'Comprar Agora' com link de checkout e foto", status: "in_progress", priority: "Alta", observations: "Em finalização — depende da integração WhatsApp Business API" },
      { number: 77, functionality: "Follow-up automático de carrinho abandonado 24h após envio do link", status: "in_progress", priority: "Alta", observations: "Em finalização — integrado com WF-02" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { number: 86, functionality: "Sugestão por ambiente: sala (rack+painel+mesa centro), quarto (cama+colchão+criado)", status: "in_progress", priority: "Alta", observations: "Lógica de sugestão por ambiente em finalização via Claude Code" },
      { number: 87, functionality: "Upsell: cama → colchão; colchão → travesseiro; rack → painel", status: "in_progress", priority: "Alta", observations: "Regras de upsell no prompt do agente em ajuste final" },
      { number: 88, functionality: "Apresentação de conjuntos completos por ambiente", status: "in_progress", priority: "Média", observations: "Em finalização — combos configuráveis por loja" },
    ],
  },
  {
    name: "14. Logística",
  ...
  },
  {
    ...
    items: [
      { number: 93, functionality: "Mensagem pós-entrega verificando se tudo ocorreu bem", status: "in_progress", priority: "Média", observations: "Template de mensagem em finalização" },
      { number: 94, functionality: "Mensagem específica pós-colchão ('Como foi sua primeira noite?')", status: "in_progress", priority: "Média", observations: "Template específico por categoria em ajuste" },
      { number: 95, functionality: "Mensagem específica pós-móvel ('Como ficou o ambiente?')", status: "in_progress", priority: "Média", observations: "Template específico por categoria em ajuste" },
      { number: 96, functionality: "Solicitação de avaliação da compra e da loja", status: "in_progress", priority: "Média", observations: "Integrado com fluxo NPS existente" },
      { number: 97, functionality: "Oferta de ajuda para complementar o ambiente", status: "in_progress", priority: "Baixa", observations: "Vinculado ao cross-sell pós-venda" },
    ],
  },
  {
