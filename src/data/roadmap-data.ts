...
export const changelog: ChangelogEntry[] = [
  { date: "2026-04-16T21:30:00", title: "🐛 Pendente: Agente não consulta catálogo", description: "Em conversas reais (cama, mesa) o agente responde 'tivemos um probleminha técnico' em vez de buscar produtos. Tool buscar_produto não está sendo invocada ou falha silenciosamente. Investigar: logs agent-tools, embedding da query, match_produtos retornando vazio.", type: "fix" },
  { date: "2026-04-16T20:00:00", title: "🐛 Fix WF-01: expressão n8n órfã no systemMessage", description: "Expressão {{ .item.json.formas_pagamento }} sem referência de node causava 'invalid syntax' no AI Agent. Corrigida para {{ $('Code - Checar Horário + Montar Contexto do Agente').item.json.formas_pagamento }}.", type: "fix" },
  { date: "2026-04-16T19:30:00", title: "🐛 Fix WF-01: tools HTTP sem valueProvider", description: "Tools cadastrar_lead, mover_pipeline, agendar_visita_loja e agendar_follow_up tinham parâmetros sem value/valueProvider, rejeitados pelo LangChain. Corrigido com modelRequired + placeholderDefinitions.", type: "fix" },
  { date: "2026-04-16T19:00:00", title: "🐛 Fix WF-01: acesso json[0] vs .json", description: "Nodes seguintes ao Supabase liam json[0].id em vez de .json.id, causando undefined em cascata. Padronizado.", type: "fix" },
  { date: "2026-04-16T18:30:00", title: "🐛 Fix WF-01: on_conflict em mensagens_processadas", description: "Node 'Marcar MessageId como Processado' agora envia ?on_conflict=message_id explícito na URL, garantindo idempotência.", type: "fix" },
  { date: "2026-04-16T18:00:00", title: "🔧 Migration: unique index leads(loja_id, telefone)", description: "Criado leads_loja_telefone_uidx para suportar upsert ON CONFLICT do WF-01. Resolve 'no unique or exclusion constraint matching'.", type: "infra" },
